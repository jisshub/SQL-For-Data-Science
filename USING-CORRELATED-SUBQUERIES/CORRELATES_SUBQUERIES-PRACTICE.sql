...
-- QUESTION - 1
-- The following correlated subqueries retrive ord_num, ord_amount, cust_code and agent_code from the 
-- table orders ( 'a' and 'b' are the aliases of orders and agents table) with following conditions -
-- the agent_code of orders table must be the same agent_code of agents table and agent_name of agents 
-- table must be Alex,
...
-- return all customer whose opening amt > max receive payment_amount of all customers.
select * from customer c1 where OPENING_AMT > 
	(select max(PAYMENT_AMT) from customer c2 where c1.CUST_COUNTRY=c2.CUST_COUNTRY);
